...
[
  ...
  {"id": 3, "card_name": "Ignite", "elephant": "Elé Phlambé", "element": "Fire", "is_conjury": false, "priority": 4, "spell_types": ["attack", "response", "boost"], "notfirst": 1, "notlast": 0, "resolve_effects": [{"condition": {"type": "if_caster_has_active_spell_of_type", "parameters": {"spell_type": "attack", "count": 1, "exclude_self": true}}, "action": {"type": "damage", "target": "prompt_enemy", "parameters": {"value": 1}}}], "advance_effects": [{"condition": {"type": "always"}, "action": {"type": "advance", "target": "this_spell"}}]},
  ...
  {"id": 5, "card_name": "Flow", "elephant": "Spout Snout", "element": "Water", "is_conjury": false, "priority": 3, "spell_types": ["remedy", "response", "boost"], "notfirst": 2, "notlast": 0, "resolve_effects": [{"condition": {"type": "if_caster_has_active_spell_of_type", "parameters": {"spell_type": "remedy", "count": 1, "exclude_self": true}}, "action": {"type": "heal", "target": "self", "parameters": {"value": 1}}}], "advance_effects": [{"condition": {"type": "always"}, "action": {"type": "advance", "target": "this_spell"}}]},
...
class ActionHandler:
    ...
    def _fire_event(self, event_type, gs, **kwargs):
        event = {"clash": gs.clash_num, "type": event_type}; event.update(kwargs); gs.event_log.append(event)

    def execute(self, action_data, gs, caster, current_card):
        action_type = action_data.get('type')

        if action_type == 'player_choice':
            if caster.is_human:
                valid_options = [opt for opt in action_data['options'] if self._resolve_target(opt, gs, caster, current_card)]
                if len(valid_options) == 1:
                    gs.action_log.append(f"Only one valid option for [{current_card.name}]. Automatically resolving '{valid_options[0].get('label')}'."); self.engine._pause()
                    self.execute(valid_options[0], gs, caster, current_card); return
                elif not valid_options: gs.action_log.append(f"{Colors.GREY}No valid options for [{current_card.name}].{Colors.ENDC}"); return
                else:
                    options_dict = {i+1: opt for i, opt in enumerate(valid_options)}; choice_idx = self.engine._prompt_for_choice(caster, options_dict, "Choose an effect to resolve:", view_key='label')
                    if choice_idx is not None: self.execute(options_dict[choice_idx], gs, caster, current_card)
                    return
            else:
                valid_options = [opt for opt in action_data['options'] if self._resolve_target(opt, gs, caster, current_card)]
                if valid_options: self.execute(random.choice(valid_options), gs, caster, current_card)
                return

        params = action_data.get('parameters', {}); targets = self._resolve_target(action_data, gs, caster, current_card)
        if not targets: gs.action_log.append(f"{Colors.GREY}No valid targets for {action_type}.{Colors.ENDC}"); return
        ...
        for target in targets:
            if action_type == 'damage':
                 ...
                 for t in target:
                     if isinstance(t, Player):
                         if not t.is_invulnerable:
                            ...
                            if t.health <= 0:
                                if self.engine._handle_trunk_loss(t) == 'game_over': return
                ...
                gs.action_log.append(f"{caster.name}'s [{current_card.name}] healed {target.name} for {params.get('value', 1)}. ({target.health}/{target.max_health})")
            ...
            # --- THIS IS THE RESTORED LOGIC ---
                ...
                self._fire_event('player_weakened', gs, player=caster.name, target=target.name, value=params.get('value', 1), card_id=current_card.id)
            
            elif action_type == 'bolster':
                target.max_health += params.get('value', 1)
                gs.action_log.append(f"{caster.name}'s [{current_card.name}] bolstered {target.name}. Max health now {target.max_health}.")
                self._fire_event('player_bolstered', gs, player=target.name, value=params.get('value', 1), card_id=current_card.id)
            # --- END OF RESTORED LOGIC ---

            elif action_type == 'discard_from_hand':
                if target.hand:
                    if target.is_human:
                        choice = self.engine._prompt_for_choice(target, {i+1: c for i, c in enumerate(target.hand)}, "Choose a card to discard:")
                        card_to_discard = target.hand.pop(choice-1)
                    else:
                        card_to_discard = min(target.hand, key=lambda c: int(c.priority) if str(c.priority).isdigit() else 99) if target.hand else None
                        if card_to_discard: target.hand.remove(card_to_discard)
                    
                    if card_to_discard:
                        target.discard_pile.append(card_to_discard)
                        gs.action_log.append(f"{caster.name} forced {target.name} to discard [{card_to_discard.name}].")
                        self._fire_event('player_discarded_card', gs, player=target.name, card_id=card_to_discard.id, source_player=caster.name)
                else: gs.action_log.append(f"{target.name} had no cards to discard.")
    ...
    def _resolve_target(self, action_data, gs, caster, current_card): # (This method is unchanged from the last working version)
...
class AI_Player:
    def choose_card_to_play(self, player, gs):
        hand_with_indices = list(enumerate(player.hand)) # e.g., [(0, CardA), (1, CardB)]
...
class GameEngine:
    ...
    def _run_resolve_phase(self) -> None:
        ...
        for spell in active_spells:
            ...
            resolve_order.append({'p_val': p_val, 'caster_idx': caster_idx, 'played_card': spell})
    ...
    def _run_advance_phase(self) -> None:
        ...
        if not advancing_spells: self.gs.action_log.append("No spells to advance this clash."); return
